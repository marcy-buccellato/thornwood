...
jQuery(document).ready(function($) {
	    	...
	    	$checkbox.unbind('change').change(function() {
		    	if($(this).is(':checked')) {
			    	...
			    	//load thumbnail from vimeo
			    		...
			    		toggleAjaxLoading();
				    	var vimeoId = $this.val().match(/http:\/\/(www\.)?vimeo.com\/(\d+)($|\/)/);
						vimeoId = vimeoId[2];
	...
	$('#update-media').click(function() {
		...
		var ids = $('input[name*=ids]').serializeArray(),
			titles = $('input[name*=titles]').serializeArray();

		$.ajax({
			url: options.Ajax_Url,
			dataType: 'json',
			type: 'post',
			data: { action: 'updatefiles', albumId: currentAlbum, ids: ids, titles: titles},
			success: function(data) {
				toggleAjaxLoading();
				showSuccessBox('Media successfully updated!');
		  }
